...
The original review (Codex) proposed a 15-item priority list, correct in content but written without knowledge of the dual-framework issue. Building token rotation, a cache policy, or a system-design doc _before_ resolving which framework is canonical means doing some of that work twice — once for the module that turns out to be the one that survives, and wasted effort on the one that gets deleted. So this roadmap reorders around that constraint.
...
| 9   | Fix remaining pagination inconsistencies (old page-number-based cursor parsing in feed routes, if any remain after migration) | Re-verify this against the _NestJS_ feed service once ported — the Express-side issue may or may not carry over depending on how migration is done. |
...
| 11  | OpenAPI/Swagger for all APIs                                                                                | Best done _after_ the NestJS migration, since NestJS has first-class OpenAPI decorator support (`@nestjs/swagger`) — doing this against the Express routes now means redoing it.                                                         |
...
Move items between phases as reality changes, and cross items off with a date and a link/reference to the change, rather than deleting them — this document is also a changelog of engineering priorities over time, useful for anyone (human or AI) trying to understand _why_ the codebase looks the way it does at any given point.
