...
async function getReactionBreakdown(quoteId) {
    try {
        const [breakdown, total] = await Promise.all([
            (0, redis_utils_1.cacheHGetAll)(redis_utils_1.RedisKeys.reactionBreakdown(quoteId)),
            (0, redis_utils_1.cacheGet)(redis_utils_1.RedisKeys.reactionTotal(quoteId)),
        ]);
        if (!total || Object.keys(breakdown).length === 0) {
            ...
            if (repairedTotal > 0) {
                await redis_utils_1.redis
                    .pipeline()
                    .hmset(redis_utils_1.RedisKeys.reactionBreakdown(quoteId), repairedBreakdown)
                    .set(redis_utils_1.RedisKeys.reactionTotal(quoteId), repairedTotal)
                    .expire(redis_utils_1.RedisKeys.reactionBreakdown(quoteId), CACHE_TTL_SECONDS)
                    .expire(redis_utils_1.RedisKeys.reactionTotal(quoteId), CACHE_TTL_SECONDS)
                    .exec();
            }
