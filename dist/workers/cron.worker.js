...
const worker = new bullmq_1.Worker(
  ...
  async (job) => {
    ...
    if (job.name === "daily-quote-of-day") {
      const latestQuote = await quote_model_1.default
        .findOne({ isHiddenBySystem: false })
        .sort({ createdAt: -1 })
        .lean();
      if (!latestQuote) {
      ...
      const recipients = await user_model_1.default
        .find({
    ...
    if (job.name === "content-sync-trigger") {
      await (0, contentSync_queue_1.addContentSyncJob)({
        type: "content-sync",
      });
...
worker.on("error", (error) => {
  logger_util_1.default.error("Scheduled cron worker error", {
    queue: queueName,
    error,
  });
});
...
(async () => {
  try {
    await (0, scheduledCron_queue_1.addScheduledJob)(
      "daily-quote-of-day",
      {},
      {
    ...
    logger_util_1.default.error("Failed to add repeatable scheduled jobs", {
      error,
    });
  }
})();
