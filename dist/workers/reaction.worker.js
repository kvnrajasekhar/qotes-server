  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      try {
        ...
        const quoteObjId = new mongoose_1.default.Types.ObjectId(
          payload.quoteId,
        );
        if (payload.action === "added") {
          const existing = await reaction_model_1.default.findOneAndUpdate(
          ...
          if (!existing) {
            ...
            await quote_model_1.default.findByIdAndUpdate(quoteObjId, {
              $inc: update,
            });
          ...
          if (payload.type === "like") update.likes = 1;
          await Promise.all([
            reaction_model_1.default.updateOne(
              { user: userObjId, quote: quoteObjId },
              { type: payload.type },
            ),
            quote_model_1.default.findByIdAndUpdate(quoteObjId, {
              $inc: update,
            }),
          ]);
          ...
          logger_util_1.default.warn(
            "Received unsupported reaction action",
            meta,
          );
        }
      } catch (err) {
        logger_util_1.default.error(
          "Kafka reaction processing failed, sending to DLQ",
          {
            ...meta,
            error: err,
          },
...
runReactionWorker().catch((err) => {
  logger_util_1.default.error("Reaction worker failed to start", {
    error: err,
  });
  process.exit(1);
});
