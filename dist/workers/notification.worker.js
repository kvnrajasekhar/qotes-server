...
const notification_service_1 = __importDefault(require("../modules/notifications/notification.service"));
...
const worker = new bullmq_1.Worker(queueName, async (job) => {
    ...
    const { type, recipientId, actorId, quoteId, email, link, subject, body, html, text, } = job.data;
    switch (type) {
        ...
        case "quote-like": {
            ...
            const recipient = await user_model_1.default.findById(recipientId).lean();
            ...
            if (!recipient || !actor || !quote) {
                throw new Error("Quote-like notification actor/recipient/quote missing");
            }
            try {
                ...
                logger_util_1.default.error("Failed to create in-app notification for quote-like", {
            ...
            logger_util_1.default.info("Skipping email for quote-like notification because recipient has no email", {
                recipientId,
                quoteId,
            });
        ...
        case "user-follow": {
            ...
            const actor = await user_model_1.default.findById(actorId).lean();
            if (!recipient || !actor) {
                throw new Error("User-follow notification actor or recipient missing");
            }
            try {
                await notification_service_1.default.createNotification({
                    recipient: recipientId,
                ...
                logger_util_1.default.error("Failed to create in-app notification for user-follow", {
                    error: notifError.message,
                });
            }
            ...
            logger_util_1.default.info("Skipping email for follow notification because recipient has no email", {
                recipientId,
            });
            return { skipped: true, emailSkipped: true };
        }
        case "generic-email": {
            ...
            return (0, email_service_1.sendEmail)({
                ...
                html: html || body || `<p>${body || "You have a new notification."}</p>`,
...
worker.on("error", (error) => {
    logger_util_1.default.error("Notification worker error", { queue: queueName, error });
});
