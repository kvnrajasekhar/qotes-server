...
const reactionService = {
  toggleReaction: async ({ userId, quoteId, type }) => {
    const allowed = await redis_utils_1.redis.slidingWindowRateLimit(
      redis_utils_1.RedisKeys.rateLimitBurst(userId),
      redis_utils_1.RedisKeys.rateLimitSustain(userId),
      Date.now(),
      10000,
      5,
      3600000,
    ...
    const updatePromises = [
      (0, reaction_cache_1.atomicUpdateCache)(quoteId, type, delta, oldType),
    ];
    if (action === "removed") {
  ...
  getQuoteReactions: async ({
    ...
    const { breakdown, total } = await (0,
    reaction_cache_1.getReactionBreakdown)(quoteId);
    const firstPageKey = redis_utils_1.RedisKeys.firstPageReactions(
      quoteId,
      viewerId || "guest",
    );
    if (!cursor && !type) {
    ...
    if (viewerId) {
      ...
      if (followingIds.length === 0) {
        followingIds = await follow_model_1.default
    ...
    if (!cursor && !type) {
      redis_utils_1.redis
        .setex(firstPageKey, 30, JSON.stringify(result))
        .catch((err) => {
