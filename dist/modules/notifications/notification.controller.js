...
const getNotifications = (0, express_async_handler_1.default)(
  async (req, res) => {
    ...
    const result = await notification_service_1.default.getNotifications(
      userId,
      {
        page: parsedPage,
        limit: parsedLimit,
...
const getUnreadCount = (0, express_async_handler_1.default)(
  async (req, res) => {
    const userId = req.user.userId;
    const unreadCount =
      await notification_service_1.default.getUnreadCount(userId);
    return (0, responseFormatter_util_1.successResponse)(
      res,
      200,
...
const markAsRead = (0, express_async_handler_1.default)(async (req, res) => {
  ...
  try {
    const notification = await notification_service_1.default.markAsRead(
      notificationId,
      userId,
    );
    return (0, responseFormatter_util_1.successResponse)(
...
const deleteNotification = (0, express_async_handler_1.default)(
  async (req, res) => {
    const userId = req.user.userId;
    const notificationId = req.params.id;
    try {
      const notification =
        await notification_service_1.default.deleteNotification(
          notificationId,
          userId,
        );
      return (0, responseFormatter_util_1.successResponse)(
