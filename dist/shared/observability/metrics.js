...
exports.toPrometheus = exports.getMetricsSnapshot = exports.observeRequest = void 0;
const metrics = {
    startedAt: Date.now(),
    requestsTotal: 0,
    responsesByStatus: {},
...
const toPrometheus = (snapshot) => {
    ...
    Object.entries(snapshot.responsesByStatus).forEach(([statusFamily, count]) => {
        lines.push(`qotes_http_responses_total{status_family="${statusFamily}"} ${count}`);
    });
    lines.push("# HELP qotes_http_route_requests_total Total HTTP requests grouped by method and route.", "# TYPE qotes_http_route_requests_total counter");
