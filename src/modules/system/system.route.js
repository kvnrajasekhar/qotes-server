...
/**
 * Comprehensive route listing for the API
 */
...
/**
 * GET /v1/system/test
 * Test route to verify system router is loaded
 */
router.get("/test", (req, res) => {
  return successResponse(res, 200, "System router is working!", {
 ...
 * Returns all available routes with descriptions
...
router.get("/routes", (req, res, next) => {
  ...
  if (!apiKey || apiKey !== process.env.API_KEY_SECRET) {
    return errorResponse(res, 403, "Invalid or missing API key");
  }

  return successResponse(res, 200, "Available API routes", {
 ...
 * Extended health check with worker status
 * Protected by API key
 */
router.get("/health", (req, res) => {
  ...
  return successResponse(res, 200, "System health", healthStatus);
});
